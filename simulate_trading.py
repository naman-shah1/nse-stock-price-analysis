...
print("Loading hourly intraday data for exact exit execution...")
hourly_df = pd.read_parquet("nse_hourly_data.parquet")
# Ensure Datetime index is sorted and timezone-naive for easy comparison
hourly_df.index = pd.to_datetime(hourly_df.index).tz_localize(None)

print(f"Downloading daily data for {len(symbols)} symbols...")
dl_start = (pd.to_datetime(START_DATE) - pd.Timedelta(days=100)).strftime('%Y-%m-%d')
...
for i, current_day in enumerate(tqdm(trading_days)):
    ...
    for strat_name, strat_config in STRATEGIES.items():
        ...
        for sym, holding in state["holdings"].items():
            ...
            # Use High-Fidelity Hourly Data to determine EXACT chronological order of hits
            # Filter hourly data for this symbol and this exact day
            day_str = current_day.strftime('%Y-%m-%d')
            mask = (hourly_df['symbol'] == sym) & (hourly_df.index.strftime('%Y-%m-%d') == day_str)
            symbol_hourly = hourly_df[mask].sort_index()
            
            if symbol_hourly.empty:
                # Fallback to daily check if hourly data is missing for some reason
                ...
                for time_idx, row in symbol_hourly.iterrows():
                    # Update trailing stop dynamically intraday as price rises
                    holding["highest_price"] = max(holding["highest_price"], row["High"])
                    dynamic_sl = holding["highest_price"] * (1 - trailing_sl_pct)
                    
                    if row["High"] >= tp_price:
                        sell_price = tp_price
                        reason = f"TP Hit at {time_idx.strftime('%H:%M')}"
                        break
                    elif row["Low"] <= dynamic_sl:
                        sell_price = dynamic_sl
                        reason = f"Trailing SL Hit at {time_idx.strftime('%H:%M')}"
                        break
        ...
        for p in buy_candidates:
            ...
            shares_to_buy = int(allocation // p["curr_open"])
            if shares_to_buy > 0:
                cost = shares_to_buy * p["curr_open"]
                state["purse"] -= cost
                sector_allocations[sec] = curr_sec_alloc + cost
                
                state["holdings"][p["symbol"]] = {
                    "buy_date": current_day,
                    "buy_price": p["curr_open"],
                    "highest_price": p["curr_open"], # Initialize trailing peak
...
trades_df.to_csv("simulation_trades_risk_managed.csv", index=False)
print("\nDetailed trades saved to simulation_trades_risk_managed.csv")
